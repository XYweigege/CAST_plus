...
- [四、数据模型](#四数据模型)
  - [4.1 实体关系](#41-实体关系)
  - [4.2 `topic` 主题词表](#42-topic-主题词表)
  - [4.3 `feedback` 客户反馈表](#43-feedback-客户反馈表)
  - [4.4 `alert` 预警表](#44-alert-预警表)
- [五、接口总览](#五接口总览)
- [六、主题词接口 `/api/topics`](#六主题词接口-apitopics)
- [七、客户反馈接口 `/api/feedbacks`](#七客户反馈接口-apifeedbacks)
- [八、预警中心接口 `/api/alerts`](#八预警中心接口-apialerts)
- [九、系统与实时通知接口](#九系统与实时通知接口)
- [十、异步分析主流程](#十异步分析主流程)
- [十一、AI 层设计](#十一ai-层设计)
- [十二、去重 / 预警 / 实时推送](#十二去重--预警--实时推送)
- [十三、配置说明](#十三配置说明)
- [十四、已知缺口与后续优化](#十四已知缺口与后续优化)
...
### 6.2 AI 扩展 POST `/api/topics/{id}/expand`
...
### 6.3 人工确认 PATCH `/api/topics/{id}/approve`
...
### 7.2 概览统计 GET `/api/feedbacks/stats`
...
### 7.3 评分归因报告 GET `/api/feedbacks/insight`
...
### 7.4 单条即时分析 POST `/api/feedbacks/analyze`
...
### 7.5 批量导入 POST `/api/feedbacks/import`
...
### 7.6 生成演示数据 POST `/api/feedbacks/generate-demo`

请求体（可选）：`{"count": 60}`，范围 1~500，超范围抛 400。
返回 `{"created": N}`。语料库每条自带人工标注，既是演示数据也是潜在的评测集。

### 7.7 人工复核 PATCH `/api/feedbacks/{id}/review`
...
### 7.8 删除 DELETE `/api/feedbacks/{id}`
...
### 9.1 SSE 通道 GET `/api/notify/stream`
...
### 9.2 手动分析 POST `/api/check-feedbacks`

返回 `{"message": "...", "queued": N}`，`queued` 为投递到 MQ 的分析任务数。结果由消费者异步产生，经 SSE 实时到达（见第十章）。
